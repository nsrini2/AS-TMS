...
it "should destroyed associated marketing_image details" do
  @marketing_message = MarketingMessage.new(@marketing_message.attributes)
...
MarketingMessage.should_receive(:find).with(:all, {:conditions=> ["active = ?", true ]})  
MarketingMessage.active_messages
end
 
 it "should get the random_active_message" do
MarketingMessage.active_messages.sample
 MarketingMessage.active_messages.should_not be_nil
