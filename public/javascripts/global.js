...
$(document).ready(function() {
  ...
  $('#primary')
    ...
    .find('.group_post_item')
        ...
        // Misc
        .find('a.vote').setup_vote_post_links().end()
...
$.fn.extend({
  ...
  setup_status_update_from_widget: function() {
    return this.bind('submit', function() {
      ...
      //   return false;
      // }
      if(status_body.length > status_maxlength) {
      ...
      $.ajax({ url: $this.attr('action'), dataType: 'html', type: 'post', data: $this.serialize(),
        ...
        success: function(data) {
          if ( data == "Not Saved" ) {
            ...
            // 
            // $('#status_body').val("");
            $('#status_body').update_status_widget(data);
          }
        },
        complete: function() {
          $this.find('input.button').each(function() {
...
/*  populate_report_preview: function() {
  ...
  setup_vote_post_links: function() {
    return this.live('click', function(event) {
      var $this = $(this); 
      var $vote = $this.parents('.vote_wrap:first');
      $vote.fadeTo('fast', 0);
      $.ajax({ 
